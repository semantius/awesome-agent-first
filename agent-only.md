...
- [headless-erp](https://github.com/Paulpandian-ai/headless-erp) - ERP exposing sixty-five typed business operations over MCP and A2A, with an HTTP facade mirroring the same surface, every write split into a simulate step and a commit returning an Ed25519-signed receipt, and approvals routed to human-only tokens. Ships no interface, in its own words "CLI tools only". (`Apache-2.0`, `self-host`, `MCP`, `REST`)
